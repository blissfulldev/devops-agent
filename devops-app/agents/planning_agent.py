...
from utils.system_prompts import PLANNING_AGENT_SYSTEM_PROMPT
from utils.executor_builder import build_function_calling_executor

async def get_planning_agent(llm):
    client = MultiServerMCPClient({
        "core_mcp": {
            "transport": "sse",
            "url": "http://localhost:8000/sse"   # core MCP HTTP endpoint
        }
    })
    ...
    async def planning_agent(state) -> Command[Literal["supervisor_agent"]]:
        ...
        prompt_query = f"{PLANNING_AGENT_SYSTEM_PROMPT}\n\n{query}"
